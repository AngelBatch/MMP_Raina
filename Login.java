...
public class Login		//US: MMP ADMIN LOGIN
...
{	
	...
	public void logInWithValid(String uname, String pword)  
	{
		try{
			...
			alp.clickOnSignIn();
			assertTrue(ahp.checkForAdminHome());
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	public void logInWithInvalid(String uname, String pword)  
	{
		try{
			...
			alp.enterUnamePword(uname, pword);
			alp.clickOnSignIn();
			assertEquals(alp.validatePopup(), "Wrong username and password. ");
			System.out.println("Error: "+ alp.validatePopup());
			alp.handlePopup();
			assertFalse(ahp.checkForAdminHome());
	...
	public void logInWithBlank()
	{
		try{
			hp.clickOnAdminLogin();
			assertTrue(alp.unameRequired());
			assertTrue(alp.pwordRequired());
		}
		catch(Exception e){
			e.printStackTrace();
		}
